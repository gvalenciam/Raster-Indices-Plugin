...
scikit_dir = os.path.join(plugin_dir, 'scikit-image')

# Checking the presence of elevation library
try:
    from skimage.filters import threshold_otsu
except:
    pip.main(['install', '--target=%s' % scikit_dir, 'scikit-image'])
    if scikit_dir not in sys.path:
        sys.path.append(scikit_dir)
...
class RasterIndices:
    ...
    def calculateNDVI(selfInstance, image):
        ...
        # Returns:
        #   NDVI Numpy array if image is multi band and band numbers exist
        #   -1 if image is single banded or band number not present in the image

        if (image.RasterCount > 1):

            try:
                nir = image.GetRasterBand(
                    selfInstance.dlg.nir_band_spinBox.value()).ReadAsArray()
            except RuntimeError:
                selfInstance.showModalDialog("Error: Band " + str(selfInstance.dlg.nir_band_spinBox.value()) + " not found in image",
                                             "Please use a different image ", 3, True, False)
                return -1

            try:
                red = image.GetRasterBand(
                    selfInstance.dlg.red_band_spinBox.value()).ReadAsArray()
            except RuntimeError:
                selfInstance.showModalDialog("Error: Band " + str(selfInstance.dlg.red_band_spinBox.value()) + " not found in image",
                                             "Please use a different image ", 3, True, False)
                return -1
            ...
            selfInstance.showModalDialog(
                "Warning: Input image is single banded", "Check if the input image has the required bands for index calculation", 2, True, False)
            return -1

    def calculateNDWI(selfInstance, image):

        # Parameters:
        #    image: Input image

        # Returns:
        #   NDWI Numpy array if image is multi band and band numbers exist
        #   -1 if image is single banded or band number not present in the image
        ...
        if (image.RasterCount > 1):
            ...
            try:
                swir1 = image.GetRasterBand(
                    selfInstance.dlg.swir1_band_spinBox.value()).ReadAsArray()
            except RuntimeError:
                selfInstance.showModalDialog("Error: Band " + str(selfInstance.dlg.swir1_band_spinBox.value()) + " not found in image",
                                             "Please use a different image ", 3, True, False)
                return -1
    ...
    def calculateWRI(selfInstance, image):
        ...
        # Returns:
        #   WRI Numpy array if image is multi band and band numbers exist
        #   -1 if image is single banded or band number not present in the image

        if (image.RasterCount > 1):

            try:
                green = image.GetRasterBand(
                    selfInstance.dlg.green_band_spinBox.value()).ReadAsArray()
            except RuntimeError:
                selfInstance.showModalDialog("Error: Band " + str(selfInstance.dlg.green_band_spinBox.value()) + " not found in image",
                                             "Please use a different image ", 3, True, False)
                return -1
            ...
            try:
                swir2 = image.GetRasterBand(
                    selfInstance.dlg.swir2_band_spinBox.value()).ReadAsArray()
            except RuntimeError:
                selfInstance.showModalDialog("Error: Band " + str(selfInstance.dlg.swir2_band_spinBox.value()) + " not found in image",
                                             "Please use a different image ", 3, True, False)
                return -1
    ...
    def __init__(self, iface):
        ...
        locale_path = os.path.join(
            self.plugin_dir,
            'i18n',
            'RasterIndices_{}.qm'.format(locale))
        ...
        self.actions = []
        self.menu = self.tr(u'&Raster Indices')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('RasterIndices', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/raster_indices/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Raster Indices'),
            callback=self.run,
    ...
    def getOtsuCheckboxSelected(self, indice):

        # Returns the otsu checkbox for the corresponding element in the 'indexes' variable

        # Parameters:
        #    indice:
        #       0 : NDVI otsu threshold checkbox
        #       1 : NDWI otsu threshold checkbox
        #       2 : WRI otsu threshold checkbox

        if (indice == 0):
            return self.dlg.ndvi_otsu_checkBox
        elif (indice == 1):
            return self.dlg.ndwi_otsu_checkBox
        elif (indice == 2):
            return self.dlg.wri_otsu_checkBox
    ...
    def calculateOtsuThreshold(self, imagePath):

        # Calculates otsu threshold value for the image located in 'imagePath'

        # Parameters:
        #    imagePath: path for the image

        # Returns:
        #   Otsu threshold numeric value (double)
        #   -1 if the image is single banded

        image = gdal.Open(imagePath)

        if (image.RasterCount == 1):
            imageArray = image.GetRasterBand(1).ReadAsArray()
            return threshold_otsu(imageArray)
        else:
            self.showModalDialog(
                "Warning: Otsu threshold image error", "Input image must have only one band", 2, True, False)
            return -1
    ...
    def startScript(self):
        ...
        # otsuthreshold = threshold_otsu(ndvi)
        # print(otsuthreshold)
            ...
            if (self.dlg.output_folder_lineEdit.text() == ''):
                ...
                for index in self.indexes:
                    ...
                    if (index["selected"] and self.processCounter >= 0):
                        ...
                        indexImageArray = index["function"](
                            self, image)

                        if (type(indexImageArray) is np.ndarray):
                    ...
                    if (self.getThresholdCheckbox(self.indexes.index(index)).isChecked() and self.processCounter >= 0):
                        ...
                        if (type(imageReclassifiedArray) is np.ndarray):
                    ...
                    if (self.getOtsuCheckboxSelected(self.indexes.index(index)).isChecked() and self.processCounter >= 0):

                        otsuThreshold = self.calculateOtsuThreshold(
                            self.imagePath)
                        imageReclassifiedArray = self.reclassify(
                            self.imagePath, otsuThreshold)

                        if (type(imageReclassifiedArray) is np.ndarray):
                            self.createTiffFromArray(image, imageReclassifiedArray, outputFolder + '/' + layerName + '_' +
                                                     'Otsu' + '_' + str(otsuThreshold) + '.tif', layerName + '_' + 'Otsu' + '_' + str(otsuThreshold) + '.tif')
                            self.dlg.progressBar.setValue(
                                self.dlg.progressBar.value() + self.singleIndexProgressValue)
                            self.processCounter += 1
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg.output_folder_pushButton.clicked.connect(
